...
def alter_tables():
    ...
    with engine.connect() as conn:
        try:
            # 如果 ancient_articles 表存在，则设置 article_id 为主键
...
def save_to_sql():
    """保存数据到数据库"""
    try:
        # 读取数据库表 ancient_articles 和 ancient_comments，如果不存在直接跳过
        if table_exists('ancient_articles'):
        ...
        # 读取本地 CSV 文件
        articleNewPd = pd.read_csv(r'D:\PythonProjects\weibo_django\Dashboard\spider_ancient\ancient_weibo_content.csv')
        commentNewPd = pd.read_csv(r'D:\PythonProjects\weibo_django\Dashboard\spider_ancient\ancient_weibo_comment.csv')
        ...
        # 删除本地 CSV 文件
        os.remove(r'D:\PythonProjects\weibo_django\Dashboard\spider_ancient\ancient_weibo_content.csv')
        os.remove(r'D:\PythonProjects\weibo_django\Dashboard\spider_ancient\ancient_weibo_comment.csv')
        print("数据保存成功")
    except Exception as e:
        print(f"保存数据时出现错误：{e}")
        # 出现错误时尝试直接写入新数据
