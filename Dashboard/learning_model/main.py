...
def first_step():
    cipingTotalFile, yuqingFile = second_step()
    # 如果数据库中不存在表，表已存在则替换
    try:
        # 如果表已存在，先读取数据库中的表
        ...
        word_frequency_new = pd.concat([word_frequency_old, cipingTotalFile]).drop_duplicates(subset='word',
                                                                                              keep='last')
        public_sentiment_new = pd.concat([public_sentiment_old, yuqingFile]).drop_duplicates(subset='comment',
                                                                                             keep='last')
        # 将处理后的数据写回数据库
